    ...
    describe('if user is authenticated', () => {
        ...
        it('expect getAllEvents() to send correct data', () => {
            return controller.getAllEvents(req, res)
                .then(() => {
                    expect(res.body.events).to.deep.include({ eventName: 1 });
                });
        });
    });

    describe('if user is authenticated but there are not available events', () => {
        beforeEach(() => {
            data = {
                events: {
                    getAllItems() {
            ...
            return controller.getAllEvents(req, res)
                .then(() => {
                    expect(res.body).to.deep.include({
                        errorMessage: 'No events avaible in this moment',
                    });
                });
        });
    });
